...
//Model 所有表格都有的基础模型
type Model struct {
	InnerID   uint64         `gorm:"column:_id;type:bigint;not null;primaryKey;" json:"_id,string"`
	ID        string         `gorm:"column:id;unique;type:char(24)" json:"id" form:"id"`                                      // 主键
...
func (model *Model) BeforeCreate(tx *gorm.DB) error {
	...
	sid := uint64(Node.Generate().Int64())
