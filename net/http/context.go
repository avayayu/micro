...
import (
	...
	"gogs.buffalo-robot.com/zouhy/micro/net/http/binding"
	"gogs.buffalo-robot.com/zouhy/micro/net/http/render"
...
func (c *Context) mustBindWith(obj interface{}, b binding.Binding) (err error) {
	...
	// 	c.FlushHttpClientError(code.RequestParamInCorrect, "无法解析参数", err)
	// 	c.Abort()
	// }
...
//GetHeader 从http请求的header中获取字段
func (c *Context) GetHeader(key string) string {
	return c.Request.Header.Get(key)
}

//CheckMysqlError 检查Mysql数据库错误 abort为true时将直接abort 否则返回构造好的 httpResponser
func (c *Context) CheckMysqlError(err error, abort bool) {

	if err == gorm.ErrRecordNotFound {
		c.FlushHttpClientError(code.HTTPClientErroCode, "数据不存在", err)
	} else {
		c.FlushHttpInnerError(code.HTTPInnerErrorCode, "数据服务出错", err)
	}
	if abort {
		c.Abort()
	}
...
func (c *Context) CheckMongoDBError(err error, abort bool) {
...
func (c *Context) CreatedBy() string {
	return c.GetString("reqUserID")
}

func (c *Context) DeletedBy() string {
	return c.GetString("reqUserID")
}

func (c *Context) UpdatedBy() string {
	return c.GetString("reqUserID")
}
...
func (c *Context) GetQueryArray(key string) (data []string, ok bool) {
	if c.Request != nil {
		query := c.Request.URL.Query()
		if data, ok = query[key]; ok && len(data) > 0 {
			return
		}
	}
	return
