...
func parseDSN(rawdsn string) *ServerConfig {
	...
	if err != nil {
		panic(pkgerr.Wrapf(err, "http: invalid dsn: %s", rawdsn))
	}
...
type ServerConfig struct {
	...
	Timeout      ztime.Duration `dsn:"query.timeout"`
	ReadTimeout  ztime.Duration `dsn:"query.readTimeout"`
	WriteTimeout ztime.Duration `dsn:"query.writeTimeout"`
}
...
func (engine *Engine) Start() error {
	...
	if err != nil {
		return pkgerr.Wrapf(err, "http: listen tcp: %s", conf.Addr)
	}

	fmt.Printf("http: start http listen addr: %s", l.Addr().String())
	server := &http.Server{
	...
	go func() {
		if err := engine.RunServer(server, l); err != nil {
			if pkgerr.Cause(err) == http.ErrServerClosed {
				fmt.Printf("http: server closed")
				return
			}
			panic(pkgerr.Wrapf(err, "http: engine.ListenServer(%+v, %+v)", server, l))
		}
	}()
...
type Engine struct {
	...
	// resourceConfigs map[string]*auth.Resource
	// microSerice     *sys.MicroServiceDesc
...
func NewServer(conf *ServerConfig) *Engine {
	...
	engine.addRoute("GET", "/healthz", func(c *Context) {
		c.Bytes(200, "text/plain", []byte("health"))
	})
...
func (engine *Engine) SetServerMode(mode constants.ServerMode) {
	engine.Mode = mode
}

func (engine *Engine) GetServerMode() constants.ServerMode {
	return engine.Mode
}
...
func (engine *Engine) addRoute(method, path string, handlers ...HandlerFunc) {
	...
	if method == "" {
		panic("httpserver: HTTP method can not be empty")
	}
	if len(handlers) == 0 {
		panic("httpserver: there must be at least one handler")
	}
...
func (engine *Engine) SetConfig(conf *ServerConfig) (err error) {
	if conf.Timeout <= 0 {
		return errors.New("http: config timeout must greater than 0")
	}
	if conf.Network == "" {
		conf.Network = "tcp"
	}
...
func (engine *Engine) Shutdown(ctx context.Context) error {
	...
	return pkgerr.WithStack(server.Shutdown(ctx))
}
...
func (engine *Engine) Run(addr ...string) (err error) {
	...
	fmt.Println("micro service now listenning on address ", address, " ", addr)
	engine.server.Store(server)
	if err = server.ListenAndServe(); err != nil {
		panic(err)
		// err = pkgerr.Wrapf(err, "addrs: %v", addr)
	}
	return
}
...
func (engine *Engine) RunTLS(addr, certFile, keyFile string) (err error) {
	...
	if err = server.ListenAndServeTLS(certFile, keyFile); err != nil {
		err = pkgerr.Wrapf(err, "tls: %s/%s:%s", addr, certFile, keyFile)
	}
	return
}
...
func (engine *Engine) RunUnix(file string) (err error) {
	...
	if err != nil {
		err = pkgerr.Wrapf(err, "unix: %s", file)
		return
	}
...
func (engine *Engine) RunServer(server *http.Server, l net.Listener) (err error) {
	...
	if err = server.Serve(l); err != nil {
		err = pkgerr.Wrapf(err, "listen server: %+v/%+v", server, l)
		return
	}
	return
}
