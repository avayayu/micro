...
func newCgroupCPU() (cpu *cgroupCPU, err error) {
	...
	if err != nil || cores == 0 {
		...
		if err != nil {
			err = fmt.Errorf("perCPUUsage() failed!err:=%v", err)
			return
		}
		cores = len(cpus)
	}
	...
	sets, err := cpuSets()
	if err != nil {
		err = fmt.Errorf("cpuSets() failed!err:=%v", err)
		return
	}
	...
	if err == nil && cq != -1 {
		...
		if period, err = cpuPeriod(); err != nil {
			err = fmt.Errorf("cpuPeriod() failed!err:=%v", err)
			return
		}
	...
	if err != nil {
		err = fmt.Errorf("systemCPUUsage() failed!err:=%v", err)
		return
	}
	preTotal, err := totalCPUUsage()
	if err != nil {
		err = fmt.Errorf("totalCPUUsage() failed!err:=%v", err)
		return
	}
	cpu = &cgroupCPU{
		frequency: maxFreq,
...
func systemCPUUsage() (usage uint64, err error) {
	...
	for err == nil {
		...
		case "cpu":
			if len(parts) < 8 {
				err = pkgerr.WithStack(fmt.Errorf("bad format of cpu stats"))
				return
			}
			...
			for _, i := range parts[1:8] {
				...
				if v, err = strconv.ParseUint(i, 10, 64); err != nil {
					err = pkgerr.WithStack(fmt.Errorf("error parsing cpu stats"))
					return
				}
				totalClockTicks += v
			}
